...
def build_s2_missing_prompt(missing_keys) -> str:
    ...
    lines.append("Alternatively, Set A can be used: CRP, TNFR1, suPAR, and SpO2 on room air.")
    return "\n".join(lines)
...
- After S1 returns "Other", suggest that the user provide additional information in the form of a validated lab set for S2; when a validated S2 set plus room-air SpO2 is present, say:
...
# S2 validated sets (plus SpO2 on room air)
# ----
Set A: CRP, TNFR1, suPAR, oxy.ra
Set B: CRP, CXCL10, IL6, oxy.ra
Full-panel allowed if many labs are present (~6+). When asking for labs, prefer Set B by default (typical priority: SpO2 on room air, CRP, IL-6, CXCL10).
...
- After any `update_sheet` tool call, your user-visible message **must end with exactly one directive**:
  ...
  - If S1 has been run and the decision is **Other**:
      - If Set A or Set B (with room-air SpO2) is complete -> say: *If you're ready, press **Run S2** with Set A/B*.
      - Otherwise, issue {"action":"ask"} for **exactly one** missing item (prefer Set B defaults: SpO2 on room air, CRP, IL-6, CXCL10).
...
def extract_features(text: str):
    ...
    # SpO2
    m = re.search(r"(SpO2|SpO2|sats?|oxygen|sat)[^\d]{0,6}([0-9]{2,3})\s*%?", t, re.I)
...
def run_s2_click(history, st):
    ...
    if vname is None and not st.get("awaiting_unvalidated_s2"):
        ...
        warn = ("Warning: this biomarker combination is NOT VALIDATED. Results may be unreliable. "
                ...
                "(A: CRP+TNFR1+suPAR+SpO2 RA; B: CRP+CXCL10+IL6+SpO2 RA).")
